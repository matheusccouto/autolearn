...
class TestGroupManagement:
    ...
    def test_set_group(self):
        ...
        dict_ = self.feat._set_group(self.feat._features)
        np.testing.assert_equal(dict_["whole"], self.feat._features)

    def test_get_extra_feats(self):
        """ Test _get_extra_feats method. """
        fake_feat = ["Fake Feature"]
        group = {"whole": list(self.feat._features) + fake_feat}
        extra = self.feat._get_extra_feats(self.feat._features, group)
        np.testing.assert_equal(extra, extra)

    def test_get_missing_feats(self):
        """ Test _get_missing_feats method. """
        group = {"whole": self.feat._features[:-1]}
        missing = self.feat._get_missing_feats(self.feat._features, group)
        np.testing.assert_equal(missing, self.feat._features[-1:])
    ...
    def test_fix_groups_raises_extra(self):
        ...
        with pytest.raises(ValueError):
            fake_feat = ["Fake Feature"]
            group = {"whole": list(self.feat._features) + fake_feat}
            self.feat._fix_groups(self.feat._features, group)

    def test_fix_groups_forgotten(self):
        """ Test that _fix_groups creates the forgotten key. """
        group = {"whole": self.feat._features[:-1]}
        fixed = self.feat._fix_groups(
            self.feat._features, group, use_forgotten=True
        )
        assert fixed["forgotten"] == self.feat._features[-1:]
...
class TestEntitySet:
    ...
    def test_set_entity_set(self):
        ...
        groups = {"p1": self.feat._features[:5], "p2": self.feat._features[5:]}
